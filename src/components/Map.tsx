...
function MapComponent({
  ...
  useEffect(() => {
    ...
    markerElement.innerHTML = `
      <div style="position:relative;display:flex;align-items:center;justify-content:center;width:20px;height:20px;">
        <span style="position:absolute;inset:0;border-radius:9999px;background:rgba(200,240,37,0.28);animation:map-marker-pulse 2.1s ease-out infinite;"></span>
        <span style="position:relative;display:flex;align-items:center;justify-content:center;width:20px;height:20px;border-radius:9999px;background:#c8f025;border:2px solid rgba(0,0,0,0.28);box-shadow:0 0 20px rgba(200,240,37,0.32);"></span>
      </div>
    `;
      ...
      .setPopup(
        new maplibregl.Popup({
          ...
          `<div style="padding:2px 2px 0;">
            <p style="margin:0;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:rgba(200,240,37,0.82);font-weight:700;">Office Location</p>
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[1.75rem] border border-white/10 bg-black ${className}`}
    >
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="absolute bottom-4 right-4 z-20 inline-flex items-center gap-2 rounded-full border border-yellow-400/20 bg-[#c8f025] px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-black transition-transform duration-300 hover:scale-[1.02]"
      >
        Open Location
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 z-20 inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/60 px-3 py-2 text-[11px] font-medium text-white/90 backdrop-blur-md">
        <MapPin className="h-3.5 w-3.5 text-[#c8f025]" />
        Navigation ready
      </div>
    </div>
  );
